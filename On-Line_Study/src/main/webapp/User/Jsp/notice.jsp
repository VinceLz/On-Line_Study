...
.notice_con .notice_text .notice_tit {
	color: #000;
	font-weight: bold;
	margin: 0 4px;
}
.notice_con .notice_delete span{
	line-height: 60px;
}
.notice_con .notice_look span{
	line-height: 60px;
	float: right;
	padding-left: 10px;
}
...
<div class="notice">
    <div class="nav_bar_con">
    	<div class="notice_header">
        	<ul class="clearfix">
                <li  class="notice_active" ><a href="<%=path %>/IndexDoorServlet?method=forowdNotice"  >通知</a></li>
        <li ><a href="<%=path %>/IndexDoorServlet?method=forowdMessage" >消息</a></li>
            </ul>
        </div>
        <div class="notice_tip clearfix">
        	<span class="fl">系统自动清理三个月前的已读通知</span>
        	<ul class="fr clearfix">
            	<li><a href="#">通知设置</a></li>
                <li><a href="<%=path %>/IndexDoorServlet?method=ForowdNoticeSetReadAll">全部标记为已读</a></li>
            </ul>
        </div>
        <div class="notice_list">
        <c:if test="${fn:length(comment)==0}">
    	<center><p>暂无通知消息!!</p></center>
    	</c:if>
         <c:forEach items="${comment}" var="comment">
	                <div class="notice_con clearfix">
	            	<a href="#" class="notice_tag fl">论坛</a>
	                <div class="notice_text fl">
	                	<p>你在<a href="<%=path %>/IndexDoorServlet?method=forowdCommentInfo&id=${comment.forum.id}&commentId=${comment.comment.id}" class="notice_tit">${comment.forum.title }</a>问题下的回答被回复<c:if test="${comment.comment.read_state==1}"><a id="notread">&nbsp;<span class="glyphicon glyphicon-comment">未读</a></c:if></p>
	                	<p>${comment.comment.comment_date }</p>
	                </div>
	                <a href="#" class="notice_look"><span class="glyphicon glyphicon-eye-open"></span></a>
	                 <a href="#" class="notice_delete"><span class="glyphicon glyphicon-trash fr"></span></a>
            </div>
         </c:forEach>  
