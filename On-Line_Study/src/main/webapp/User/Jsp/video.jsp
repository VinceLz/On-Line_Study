...
<script type="text/javascript" src="<%=path %>/User/bootstrap/js/sweet-alert.js"></script>
<script type="text/javascript" src="<%=path %>/User/bootstrap/js/sweet-alert.min.js"></script>
...
<link href="<%=path %>/User/CSS/Video/CSS/VideoPage/reset.css" rel="stylesheet">
<link href="<%=path %>/User/bootstrap/el/style.css" rel="stylesheet">
<link href="<%=path %>/User/bootstrap/css/sweet-alert.css" rel="stylesheet">
</head>
<style>
@charset "utf-8";
/* CSS Document */
...
<div class="center_video">
  <video src="<%=path %>/${resourceList.src}" width="100%" controls="controls" type="video/mp4"></video>
</div>
  ...
  <div class="row">
    <div class="col-md-9">
      <div class="three_page">
      	...
      	<h4 class="friend_title" ><span class="icon_chat" style="color: gray;"></span>评论<span ><a class="icon_download" href="<%=path %>/ResourceServlet?method=downloadFile&sid=${resourceList.id}" onclick="return checkLogin('${user}')"  id="download" style="float: right;font-size:70%;margin-right: 5%;">点击下载</a><a  class="icon_like"  href="javaScript:void()" onclick="return addSupport('${user}','${resourceList.id}')" id="download" style="float: right;font-size:70%;margin-right: 3%;">支持一下</a></span></h4>
        <div class="friend_images">
 <c:if test="${user==null }">
          <div class="textarea">
            <textarea class="form-control"  rows="2"  id="texa" placeholder="登录后才可以评论"></textarea>
          </div>
</c:if>

<c:if test="${user!=null }">
          <div class="textarea">
            <textarea class="form-control"  rows="4"  id="texa" placeholder="请输入您的评论"></textarea>
          </div>
   
          <button type="button" class="btn btn-success" onclick="getInput(${resId});">发布评论</button>
</c:if>
 <c:forEach items="${listCommentInfo}" var="listCommentInfo">                          
          <div class="media">
            <a class="media-right" href="#">
               <img class="" src="<%=path %>/${listCommentInfo.student.picture}" alt="icon1" width="50" height="50" style="border-radius:50%;"/>
            </a>
            <div class="media-body">
                <h4>${listCommentInfo.student.nickName}</h4>
                <p>${listCommentInfo.comment.content}</p>
                <p class="media_time">发表时间：${listCommentInfo.comment.comment_date}</p>                    
            </div>       
         </div>
     </c:forEach>    
    ...
    <div class="col-md-3">
      <div class="right_list">
        <ul class="list-group">
        <li class="list-group-item list_first"><h4>相关推荐</h4></li>
          <li class="list-group-item">
          <div class="class_img">
          <a href="#">
          <img src="Images/Video_Page/class.jpg"/>
          </div>
          <p>JavaScript入门</p></a>
          </li>
          <li class="list-group-item">CSS3深入</li>
          <li class="list-group-item">JavaScript高级程序设计</li>
          <li class="list-group-item">锋利的jQuery</li>
          <li class="list-group-item">JavaScript高级程序设计</li>
          <li class="list-group-item">锋利的jQuery</li>
        </ul>
      </div>
	...
	if(user==""){
	      ...
	      return false;
	    	...
	    	var xmlHttp = createXMLHttpRequest();
				xmlHttp.open("POST", "<c:url value='/ResourceServlet'/>", true);
				xmlHttp.setRequestHeader("Content-Type",
						"application/x-www-form-urlencoded");
				xmlHttp.send("method=AddSupport&id="+id);
	...
	var text=document.getElementById("texa").value;
	window.location.href="UserServlet?method=addComment&id="+id+"&content="+text;
	}
