	...
	<div class="data_list">
		...
		<div class="data_exam_content">
			<form id="myForm" action="exam!add" method="post">
				<input type="hidden" name="exam.student.id" value="当前用户" /> <input
					type="hidden" name="exam.paper.id" value="${paper.id }" /> <strong><big>一，单选题</big>
				</strong>(每题20分，答错不得分)<br /> <br />
				<c:forEach var="s" items="${squestionList}" varStatus="status">
					<strong>[&nbsp;${status.index+1 }&nbsp;]&nbsp;${s.subject
						}</strong>
					<br />
					<br />
				...
				<c:forEach var="m" items="${mquestionList}" varStatus="status">
					<strong>[&nbsp;${status.index+1 }&nbsp;]&nbsp;${m.subject
						}</strong>
					<br />
					<br />
