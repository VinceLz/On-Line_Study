            ...
            <header style="padding-top: 30px;" id="header" class="header">  
                <div class="container">            
                    <h1 class="logo pull-left">
                        <a href="">
                            <img class="logo-image" src="<%=path %>${indexModel.logoImageSrc }" alt="网站LOGO" height="50px">
                        ...
                        <div style="height: 1px;" class="navbar-collapse collapse" id="navbar-collapse">
                            <ul class="nav navbar-nav " >
                                <c:forEach items="${menuList}" var="menuList">
									<li class="nav-item"><a href="<%=path %>/${menuList.menuAddr }">${menuList.menuName }</a></li>                         
                                </c:forEach>
                            </ul><!--//nav-->
	...
	<div class="coursecon">
    	<div class="coursenav">   	
            <c:forEach items="${interestList}" var="interest">
            	<a class="courseNava" href="<%=path %>/ResourceServlet?method=showAllResource">${interest.interestName}</a>            
         	</c:forEach>
        </div>
          <c:forEach items="${resourceListList }"  var="resourceList"  >
          	<c:forEach items="${resourceList }"  var="resource" >
          		<div class="${resource.frontClassName }">
           		<a class="coursea" href="<%=path %>${resource.linkSrc }<%="&id="%>${resource.id}">
           		<img class="courseImg" src="${resource.showImageSrc }"  width="260" height="150"/>
            	<p class="coursep" style="width: 150px;">${resource.resourceName }</p>
                <p class="coursepl">${resource.uploadeDate }</p>
                <p class="coursepr"><span class="glyphicon glyphicon-thumbs-up" style="color:#ff9966;"></span>${resource.likeSum }</p>
              	</a>
              	</div>
          	</c:forEach>
          </c:forEach>
				...
				<div class="modal-body">
					...
					<div class="tab-content">
						<div class="tab-pane fade" id="Loginon">
							<form id="form_id" action="<%=path %>/UserServlet?method=userLogin"
								method="post">
								<fieldset>
									...
									<div class="form-group vcode">
										<input type="text" id="validate" name="validate"
											...
											src="<%=path %>/VerifyCodeServlet" id="img" onclick="change(this)">
						...
						<div class="tab-pane fade" id="Register">
							<form id="form_rs" action="<%=path %>/UserServlet?method=userRegister" method="Post">
								<fieldset>
									<div class="form-group" style="margin-top:20px">
										<input type="text" id="account" name="account"
									...
									<div class="form-group vcode">
										<input type="text" id="validate" name="validate"
											class="form-control" placeholder="请输入验证码"
											style="width:130px;float:left"> <img
											src="<%=path %>/VerifyCodeServlet" id="img" onclick="change(this)"> <a href="" ><span
											class="glyphicon glyphicon-refresh" aria-hidden="true"></span></a>
									</div>
									<div class="">
										<input type="submit" class="btn btn-lg btn-block loandre_btn"
