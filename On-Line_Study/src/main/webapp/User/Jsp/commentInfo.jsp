			...
			<ul>
				<li class="read">
					...
					<div class="post-element">
						<center>
							<span><a target="" href="#"><img
									src="<%=path %>${pub_user.picture }">${forum.pub_account
									}</a> </span> <span><i class="fa fa-clock-o"></i>发布日期：${forum.pub_date
								}</span> <span><i class="fa fa-eye"></i>点击量：${forum.support } </span> <span><i
								class="fa fa-commenting-o"></i>${forum.resv2}条评论</span>
						</center>
					</div>
					<div class="more">${forum.content }</div>
					<div class="row linkmore">
						<c:if test="${backForum.title!='' }">
							<div class="col-lg-6">
								<a
									href="<%=path %>/IndexDoorServlet?method=forowdCommentInfo&id=${backForum.id}">上一篇：${backForum.title
									}</a>
							</div>
						</c:if>
						<c:if test="${nextForum.title!='' }">
							<div class="col-lg-6">
								<a
									href="<%=path %>/IndexDoorServlet?method=forowdCommentInfo&id=${nextForum.id}">下一篇：${nextForum.title
									}</a>
							</div>
						</c:if>
					</div></li>
