	...
	public String login(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getSession().getAttribute("random").equals(code)) {
			...
			if (as.AdminLogin(admin)) {
				...
				request.getSession().setAttribute("UIP",
				...
				request.getSession().setAttribute("logintime",
						administrator.getLogintime());
				request.getSession().setAttribute("nakename",
						administrator.getNikename());
				request.getSession().setAttribute("grade",
						administrator.getRank());
				request.getSession().setAttribute("admin", name);
				String[] str = as.FindAdminByAccount(admin).getPicture()
						.split(";");
				request.getSession().setAttribute("img", str[str.length - 1]);
				request.getSession().setAttribute("time",
						new Date().toLocaleString());
				request.getSession().setAttribute("admin_teacher",
						administrator);
				...
				// request.getRequestDispatcher("/Admin/Error/UserNaemError.jsp").forward(request,
				// response);
			...
			// request.getRequestDispatcher("/Admin/Error/CodeError.jsp").forward(request,
			// response);
		}
